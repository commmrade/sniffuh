...
std::vector<Entry> read_file(const std::string_view filename) {
    ...
    pdor::File file{filename.data()};
    if (!file.open(pdor::Mode::Read)) {
        return result;
    }
...
Writer::Writer(int write_interval, std::string_view filename) : m_interval(write_interval), m_filename(filename), m_file(std::string{filename}) {
    m_file.open(pdor::Mode::Write);
